...
const TASK_ID = "trec-covid-no-search";
...
const topics = require('./data/covidtopics.json');
topics['topics']['topic'].forEach((item, index) => {
    item.id = index;
});

function sampleTopics(n) {
    // let validTopics = underscore.omit(topics, '0');
    let samples = utils.sample(topics['topics']['topic'], n);
    // samples[1] = topics[0];
    return samples;
}
